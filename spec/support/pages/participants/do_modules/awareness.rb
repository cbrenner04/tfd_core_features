...
module Participants
  module DoModules
    ...
    class Awareness
      ...
      def create_time_period(start_time:, end_time:)
      ...
      def complete_multiple_hour_review(review_responses)
        (0..review_responses[:hours])
          .zip(
            review_responses[:activities],
            review_responses[:pleasure_ratings],
            review_responses[:accomplishment_ratings]
          ) do |index, activity, pleasure, accomplishment|
          complete_one_hour_review(index, activity, pleasure, accomplishment)
        end
        participant_navigation.next
      end

      def complete_one_hour_review(index, activity, pleasure, accomplishment)
        fill_in "activity_type_#{index}", with: activity
        do_tool.choose_rating("pleasure_#{index}", pleasure)
        do_tool.choose_rating("accomplishment_#{index}", accomplishment)
      ...
      def has_entries?(counts:)
        %w(recent fun accomplished).zip(counts) do |type, count|
          find("##{type}_activities").has_css?('tr', count: count)
